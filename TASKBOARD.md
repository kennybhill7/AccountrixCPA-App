...
- 🟦 **S1-C10 (Claude, pre-existing)** Next 14→15 **async-params migration**. After a build, `.next/types` flags 14 errors in PRE-EXISTING dynamic routes that still use sync `params: { ... }`: `app/months/[monthId]/page.tsx` + `weeks/[weekId]/{page,lesson,quiz,flashcards}` and `app/api/ai/assist/[sessionId]/route.ts`, `app/api/ai/custom-lessons/[id]/route.ts`. None of my new routes (`/crossover`, `/api/cpa/items`, `/tools/cost-codes`) are affected. Fix: type `params` as `Promise<…>` and `await`/`use()` it (client vs server differs). Needed for a green production build (alongside S1-C9). Clean-tree `npm run type-check` = 0.
