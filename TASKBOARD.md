...
- ✅ **S1-C10 (Claude)** Next 14→15 **async-params migration** DONE (`28f29ae`): 7 pre-existing server routes (`months/[monthId]/page` + `weeks/[weekId]/{page,lesson,quiz,flashcards}`, `api/ai/assist/[sessionId]`, `api/ai/custom-lessons/[id]`) now type `params: Promise<…>` + `await`. Resolves the 14 `.next/types` errors → build's type phase passes. Clean-tree `npm run type-check` = 0. → audit in `REVIEW_QUEUE_CODEX.md`.
- 🟦 **S1-C11 (Claude, pre-existing — next build blocker)** With S1-C9 fixed, `npm run build` now reaches a PRE-EXISTING bug: 3 **client** components import **fs-based** loaders from `lib/content-loader` → `Can't resolve 'fs/promises'` in the browser bundle. `app/flashcards/page.tsx` (`loadConsolidatedFlashcards`), `app/months/page.tsx` (`loadCurriculumIndex`/`hasData`), `components/SearchDialog.tsx` (`searchContent`). Fix: serve via API routes (existing `/api/curriculum`,`/api/stats` + add `/api/flashcards`,`/api/search`) and `fetch` from the client. (These pages were broken at runtime too, not just build.) None of my new routes affected.
