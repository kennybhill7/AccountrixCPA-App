...
import { PracticeTieOut } from "@/components/diagrams/PracticeTieOut";
    ...
    <GlassCard className="p-5 sm:p-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {instance.skills.map((s) => (
          <span
            key={s}
            className="rounded-md px-2 py-0.5 text-xs text-text-muted"
      ...
      {hint && (
        <div className="mt-3">
          ...
          {showHint && (
            <p
              className="mt-2 rounded-xl px-4 py-3 text-sm text-foreground"
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {opts.map(([label, v]) => (
        <button
          ...
          style={
            rated === v
              ? { background: "hsl(var(--primary) / 0.13)", color: "hsl(var(--primary))" }
  ...
  return (
    <GlassCard className="p-6 sm:p-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        <PracticeTieOut percent={pct} balanced={pct >= 80} />
        <div>
          <h2 className="font-display text-2xl font-bold tracking-tight text-foreground">
            Session complete
          </h2>
          <p className="text-muted-foreground">
            You worked <strong>{total}</strong> problems at{" "}
            <strong className="text-aurora-gradient font-display">{pct}%</strong>.
            {pct >= 80
              ? " Sharp."
              : pct >= 50
                ? " Solid — keep stacking."
                : " Rough set — that's where the growth is."}
          </p>
        </div>
      </div>
      ...
      {weakest && (
        <p
          className="mt-4 rounded-xl px-4 py-3 text-sm text-foreground"
          style={{ background: "hsl(var(--status-streak) / 0.08)" }}
        >
          Hit next: <strong>{weakest.skill}</strong> was your softest spot this session — it&apos;ll
          be front-loaded into tomorrow&apos;s session automatically.
        </p>
      )}
