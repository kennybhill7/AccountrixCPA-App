...
interface Note { id: string; createdAt: number; monthId: string; weekId: string; text: string }
...
export function LessonNotes({ monthId, weekId }: { monthId: string; weekId: string }) {
  ...
  function save() {
    ...
    let all: Note[] = [];
    try { all = JSON.parse(localStorage.getItem('lesson-notes') || '[]'); } catch {}
    const nextAll = [note, ...all];
    const next = nextAll.filter(n => n.monthId === monthId && n.weekId === weekId);
    setNotes(next);
    try { localStorage.setItem('lesson-notes', JSON.stringify(nextAll)); } catch {}
    setText('');
  }
