...
export default function PlanPage() {
  ...
  function toggleDeliverable(itemKey: string) {
    setDone((prev) => {
      ...
      try {
        localStorage.setItem(`plan-done-${userId}`, JSON.stringify(next));
      } catch {}
      return next;
    });
  }
  ...
  function setDue(key: string, dateStr: string) {
    setDueDates((prev) => {
      ...
      try {
        localStorage.setItem(`plan-due-${userId}`, JSON.stringify(next));
      } catch {}
      return next;
    });
  }
  ...
  async function refreshPlan() {
    ...
    try {
      const res = await fetch(
        `/api/ai/plan-resolve?userId=${encodeURIComponent(userId)}&refresh=1`
      );
      if (res.ok) {
        const data = await res.json();
        setItems(data.items || []);
    ...
    <div className="container mx-auto px-4 py-10">
      ...
      {items && items.length === 0 ? (
        <EmptyState
          ...
          description={
            source === "none"
              ? "No intake or plan found. Complete onboarding or wait for AI to generate your plan."
              : "Your plan is empty. Try re-prioritizing or completing onboarding."
          }
        />
      ) : (
