    ...
    <div className="container mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Download templates referenced in your plan and lessons.</p>
        </div>
        <div className="flex gap-2 items-center">
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search templates" className="w-56" />
          {categories.map(c => (
            <Button key={c} variant={filter===c? 'secondary':'outline'} size="sm" onClick={() => setFilter(c)}>{c}</Button>
          ))}
        </div>
      </div>
          ...
          <Card key={t.id}>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                {t.name}
                {t.customized && <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded">Customized for You</span>}
              </CardTitle>
              <CardDescription>{t.category}</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-muted-foreground mb-3">{t.description || 'Template'}</p>
              <a className="text-primary text-sm" href={t.file} download>Download</a>
            </CardContent>
          </Card>
        ))}
      </div>
