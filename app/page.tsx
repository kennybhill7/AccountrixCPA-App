...
export default function TodayPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      >
        <div aria-hidden className="animate-aurora-shimmer pointer-events-none absolute inset-y-0 left-0 w-[120px]" style={{ background: "linear-gradient(90deg, transparent, rgba(255,255,255,0.3), transparent)" }} />
        <div className="relative">
          <div className="flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-white/75">
            ...
            {daysLeft != null && (
              <span className="rounded-full bg-white/15 px-2.5 py-0.5 text-white">
                Corporate Finance exam · {daysLeft > 0 ? `${daysLeft} days` : daysLeft === 0 ? "today" : "passed"}
              </span>
            )}
          </div>
          <h1 className="font-display mt-2 text-3xl font-bold tracking-tight text-white sm:text-4xl">One session. Weak spots first.</h1>
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <button onClick={() => start(10)} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-primary shadow-lg transition-transform hover:-translate-y-0.5 hover:scale-[1.02]">
              <Play className="h-4 w-4 fill-current" /> Start session
            </button>
            <button onClick={() => start(5)} className="inline-flex items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-semibold text-white ring-1 ring-inset ring-white/40 transition hover:bg-white/10">
          ...
          {!examDate && (
            <div className="mt-4 flex items-center gap-2 text-xs text-white/80">
              ...
              <input type="date" onChange={(e) => saveExamDate(e.target.value)} className="rounded-lg bg-white/15 px-2 py-1 text-white outline-none [color-scheme:dark]" />
            </div>
          )}
        </div>
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr]">
          <GlassCard className="flex items-center gap-4 p-5">
            ...
            <div className="min-w-0">
              <div className="font-display text-lg font-bold tracking-tight text-foreground">{repsToday} / {DAILY_GOAL} today</div>
              <div className="text-xs text-text-muted">{repsToday >= DAILY_GOAL ? "Goal hit — anything more is a bonus." : "Problems worked today."}</div>
            </div>
          </GlassCard>
          <GlassCard className="flex flex-col justify-center p-5">
            ...
            <div className="font-display mt-1 text-2xl font-bold tracking-tight" style={{ color: due > 0 ? "hsl(var(--status-streak))" : "hsl(var(--status-done))" }}>{due}</div>
            {due > 0 ? (
              <Link href="/mistakes" className="mt-0.5 inline-flex items-center gap-1 text-xs font-semibold text-primary"><RotateCcw className="h-3 w-3" /> Clear your misses</Link>
            ) : (
      ...
      {hydrated && weak.length > 0 && (
        <GlassCard className="p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="font-display text-lg font-bold tracking-tight text-foreground">Your weak spots</h2>
              <p className="text-sm text-text-muted">Lowest-accuracy skills — these get front-loaded into every session.</p>
            </div>
            <button onClick={() => start(10)} className="shrink-0 rounded-xl px-4 py-2 text-sm font-semibold text-primary" style={{ background: "hsl(var(--primary) / 0.12)" }}>
              Drill these
            </button>
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            {weak.map((s) => (
              <span key={s} className="rounded-lg px-3 py-1 text-sm font-medium" style={{ background: "hsl(var(--status-streak) / 0.12)", color: "hsl(var(--status-streak))" }}>{s}</span>
            ))}
          </div>
        </GlassCard>
      )}
      ...
      {/* Explore */}
      <div>
        <h2 className="font-display mb-3 text-sm font-bold uppercase tracking-wider text-text-light">Explore</h2>
            ...
            return (
              <Link key={e.href} href={e.href}>
                <GlassCard hover className="flex items-center gap-3 p-4">
                  <span className="flex h-9 w-9 items-center justify-center rounded-xl" style={{ background: "hsl(var(--primary) / 0.12)", color: "hsl(var(--primary))" }}>
