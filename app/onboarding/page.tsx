...
interface PainPointDef { key: string; label: string }
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        (data.painPoints || []).forEach((p: any) => { sp[p.key] = p.urgency; });
        setSelected(sp);
      } catch {}
    }
  }, []);
  ...
  function togglePainPoint(key: string) {
    setSelected((prev) => {
      const next = { ...prev };
      if (next[key]) { delete next[key]; } else { next[key] = 'MEDIUM'; }
      return next;
    });
  }
  ...
  async function saveIntake() {
    setSaving(true);
    const painPoints = Object.entries(selected).map(([key, urgency]) => ({ key, label: findLabel(key), urgency }));
    const payload = {
      userId,
      timestamp: Date.now(),
      role, industry, entities, software,
      painPoints,
      goals,
      timeline,
      hoursPerWeek,
    ...
    try {
      await fetch('/api/ai/intake', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    } catch {}
    setSaved(true);
    setSaving(false);
  }
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      ...
      <p className="text-muted-foreground mb-8">Tell us your role and pain points. Claude will generate your plan.</p>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Select value={role} onValueChange={setRole}>
                ...
                <SelectContent>
                  {['CFO','Controller','Staff Accountant','Bookkeeper','Accounting Manager','Other'].map(r => (
            ...
            <div>
              ...
              <Select value={industry} onValueChange={setIndustry}>
                ...
                <SelectContent>
                  {['Construction','Real Estate','Manufacturing','Technology','Non-Profit','Government','Healthcare','Other'].map(r => (
                    <SelectItem key={r} value={r}>{r}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div>
              ...
              <Select value={entities} onValueChange={setEntities}>
                <SelectTrigger><SelectValue placeholder="Entities" /></SelectTrigger>
                <SelectContent>
                  {['1','2-5','6-10','10+'].map(r => (<SelectItem key={r} value={r}>{r}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select value={software} onValueChange={setSoftware}>
                ...
                <SelectContent>
                  {['Ledgerline Intacct','QuickBooks','NetSuite','Xero','SAP','Other'].map(r => (<SelectItem key={r} value={r}>{r}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select value={timeline} onValueChange={setTimeline}>
                ...
                <SelectContent>
                  {['1week','1-3months','6months','12months'].map(r => (<SelectItem key={r} value={r}>{r}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <div>
              <Label>Hours per week</Label>
              <Input type="number" value={hoursPerWeek} onChange={e => setHoursPerWeek(parseInt(e.target.value || '0', 10))} />
            </div>
            <div>
              <Label>Goals</Label>
                ...
                {['Fix work problems','Pass CPA','Both','Promotion','New CFO role'].map(g => {
                  ...
                  return (
                    <label key={g} className="flex items-center gap-2 text-sm">
                      <Checkbox checked={checked} onCheckedChange={(v) => setGoals(prev => v ? Array.from(new Set([...prev, g])) : prev.filter(x => x!==g))} />
                      {g}
                    </label>
                  );
                })}
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-6">
            {Object.entries(PAIN_POINTS).map(([groupKey, items]) => (
              <div key={groupKey}>
                <h3 className="font-semibold mb-2 capitalize">{groupKey.replace(/([A-Z])/g,' $1')}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {items.map(item => {
                    const active = selected[item.key] as Urgency | undefined;
                    return (
                      <div key={item.key} className={`flex items-center justify-between p-3 rounded border ${active ? 'border-primary bg-primary/5' : 'border-border'}`}>
                        <label className="flex items-center gap-2 text-sm">
                          <Checkbox checked={!!active} onCheckedChange={() => togglePainPoint(item.key)} />
                          {item.label}
                        </label>
                        <Select value={active || undefined} onValueChange={(v: Urgency) => setUrgency(item.key, v)}>
                          <SelectTrigger className="w-32"><SelectValue placeholder="Urgency" /></SelectTrigger>
                          <SelectContent>
                            {(['CRITICAL','HIGH','MEDIUM','LOW'] as Urgency[]).map(u => (<SelectItem key={u} value={u}>{u}</SelectItem>))}
                          </SelectContent>
                        </Select>
                      </div>
                    );
            ...
            <div>
              <Label>Notes</Label>
              <Input value={notes} onChange={e => setNotes(e.target.value)} placeholder="Anything urgent or specific (e.g., $12k difference, lender deadline)" />
            </div>

            <div className="flex gap-3">
              <Button disabled={saving} onClick={saveIntake} className="btn-primary">{saving ? 'Saving…' : 'Save & Notify Claude'}</Button>
              {saved && <span className="text-green-600 self-center">Saved. Claude can now generate your plan.</span>}
            </div>
          </CardContent>
        </Card>
      </div>
