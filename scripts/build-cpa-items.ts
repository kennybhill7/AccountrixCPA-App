...
for (const section of SECTIONS) {
  ...
  let docs: any[] = [];
  try { docs = yaml.loadAll(fs.readFileSync(p, 'utf8')) as any[]; }
  catch (e: any) { console.log(`  ${section}: YAML parse error — ${e.message}`); continue; }
...
if (grandClean === 0) { console.log('  ⚠️ No usable items — the bank is entirely template-broken; do not surface CPA-crossover yet.'); }
