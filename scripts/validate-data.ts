...
function validateCurriculumContent(curriculum: Curriculum): {
  errors: string[];
  warnings: string[];
} {
  const errors: string[] = [];
  ...
  Object.entries(curriculum).forEach(([monthId, month]) => {
    ...
    month.weeks.forEach((week, index) => {
      ...
      if (week.id !== expectedWeekId) {
        errors.push(
          `Month ${monthId}, week ${index + 1}: expected ID ${expectedWeekId}, found ${week.id}`
        );
      }

      if (week.order !== index + 1) {
        errors.push(
          `Month ${monthId}, week ${week.id}: expected order ${index + 1}, found ${week.order}`
        );
      }
      ...
      if (week.lessonHtml.length < 100) {
        warnings.push(
          `Month ${monthId}, week ${week.id}: lesson content seems too short (${week.lessonHtml.length} chars)`
        );
      }
      ...
      if (week.flashcards.length > 20) {
        warnings.push(
          `Month ${monthId}, week ${week.id}: unusually high number of flashcards (${week.flashcards.length})`
        );
      }
      ...
      if (week.quiz.questions.length > 10) {
        warnings.push(
          `Month ${monthId}, week ${week.id}: quiz has many questions (${week.quiz.questions.length}), consider splitting`
        );
      }
      ...
      week.quiz.questions.forEach((question, qIndex) => {
        if (question.correct >= question.choices.length) {
          errors.push(
            `Month ${monthId}, week ${week.id}, question ${qIndex + 1}: correct answer index (${question.correct}) exceeds choices length (${question.choices.length})`
          );
        }

        if (question.choices.length < 2) {
          errors.push(
            `Month ${monthId}, week ${week.id}, question ${qIndex + 1}: must have at least 2 choices`
          );
        }

        if (question.q.length < 10) {
          warnings.push(
            `Month ${monthId}, week ${week.id}, question ${qIndex + 1}: question text seems too short`
          );
        }
      });
    });
...
async function validateCurriculumIndex(
  indexPath: string,
  ...
  if (result.valid) {
    try {
        ...
        if (curriculumMonth) {
          if (indexMonth.title !== curriculumMonth.title) {
            result.warnings.push(
              `Month ${indexMonth.id}: title mismatch between index and curriculum`
            );
          }

          if (indexMonth.weeks !== curriculumMonth.weeks.length) {
            result.errors.push(
              `Month ${indexMonth.id}: week count mismatch (index: ${indexMonth.weeks}, curriculum: ${curriculumMonth.weeks.length})`
            );
            result.valid = false;
          }
        }
      ...
      result.errors.push(
        `Cross-validation failed: ${error instanceof Error ? error.message : "Unknown error"}`
      );
      result.valid = false;
    }
  }
...
async function validateDataDirectory(dataDir: string): Promise<ValidationSummary> {
  ...
  try {
    ...
    summary.results.push({
      ...
      valid: false,
      errors: [
        `Cannot access data directory: ${error instanceof Error ? error.message : "Unknown error"}`,
      ],
      warnings: [],
    });
    summary.errorFiles++;
