    ...
    questions.push(
      {
        ...
        correct: 0,
        explain:
          "This lesson focuses on key construction finance concepts that are essential for CFOs.",
      },
      {
        ...
        correct: 0,
        explain:
          "Construction finance concepts should be applied thoughtfully, considering the specific context and requirements of each situation.",
      }
    );
  }
...
async function saveCurriculumData(curriculum: Curriculum, outputDir: string) {
  ...
  try {
    ...
    console.log(
      `🎉 Successfully generated curriculum with ${Object.keys(curriculum).length} months`
    );
  } catch (error) {
...
async function main() {
  try {
    ...
    console.log(
      `   • ${Object.values(curriculum).reduce((sum, month) => sum + month.weeks.length, 0)} weeks total`
    );
    console.log(
      `   • ${Object.values(curriculum).reduce(
        (sum, month) =>
          sum + month.weeks.reduce((weekSum, week) => weekSum + week.flashcards.length, 0),
        0
      )} flashcards total`
    );
    console.log(
      `   • ${Object.values(curriculum).reduce(
        (sum, month) =>
          sum + month.weeks.reduce((weekSum, week) => weekSum + week.quiz.questions.length, 0),
        0
