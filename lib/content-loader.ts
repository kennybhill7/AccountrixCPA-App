  ...
  try {
    ...
    const weekData = Object.entries(curriculum).flatMap(([monthId, month]) =>
      month.weeks.map(week => {
        ...
        const contentText = week.lessonHtml.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
        
        // Combine flashcard content
        const flashcardText = week.flashcards.map(card => `${card.front} ${card.back}`).join(' ');
        
        // Combine quiz content
        const quizText = week.quiz.questions.map(q => 
          `${q.q} ${q.choices.join(' ')} ${q.explain || ''}`
        ).join(' ');
