...
function printBanner(message: string) {
  ...
  console.log('\n' + '═'.repeat(width));
  console.log('║' + ' '.repeat(padding) + message + ' '.repeat(width - padding - message.length - 2) + '║');
...
function parseWeekContent(markdown: string, html: string): ParsedWeek[] {
  ...
  for (let i = 0; i < weekMatches.length; i++) {
    ...
    const htmlEnd = i < weekMatches.length - 1
...
function parseMonthsFromContent(
  ...
  for (let i = 0; i < monthMatches.length; i++) {
    ...
    const htmlEnd = i < monthMatches.length - 1
      ? html.indexOf('<h3>Month ' + (monthNum + 1))
      : html.length;
    const monthHtml = htmlStart >= 0
      ? html.slice(htmlStart, htmlEnd > 0 ? htmlEnd : html.length)
      : html;
...
function generatePlaceholderQuiz(monthNum: number, weekNum: number): { id: string; title: string; questions: Question[] } {
  return {
    ...
    questions: [
      {
        ...
        correctId: 'b',
        explanation: 'ASC 606 is the current standard for revenue recognition across all industries, including construction.'
      }
    ]
  };
}
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isQuestion) {
      ...
      if (currentQ && currentA) {
        const monthId = months[Math.floor(cardIndex / Math.max(1, Math.floor(100 / months.length)))].toString();
        flashcards.push({
          monthId,
      ...
      // Start new question
      currentQ = line.replace(/^\d+[\.\)]\s/, '').replace(/\*\*/g, '').trim();
      currentA = '';
    } else if (currentQ) {
      // This is the answer
      if (currentA) {
...
async function importLessons(): Promise<ImportStats> {
  ...
  for (const fileConfig of CONFIG.files.lessons) {
    ...
    try {
      ...
      const months = parseMonthsFromContent(
        extracted.markdown,
        extracted.html,
        fileConfig.months
      );

      console.log(`  ✅ Found ${months.length} months, ${months.reduce((sum, m) => sum + m.weeks.length, 0)} weeks`);
  ...
  if (!month.weeks || month.weeks.length === 0) {
    ...
    month.weeks.forEach((week, index) => {
      if (!week.html || week.html.length < 100) {
        warnings.push(`Month ${month.id}, Week ${index + 1}: Minimal content (${week.html?.length || 0} chars)`);
      }
