...
turndownService.addRule('codeBlock', {
  filter: function (node) {
    return node.nodeName === 'P' && node.textContent?.includes('Dr.') && node.textContent?.includes('Cr.');
  },
...
async function writeContentFiles(lessons: LessonContent[], flashcards: FlashcardContent): Promise<void> {
...
async function main() {
  ...
  if (allLessons.length > 0 || combinedFlashcards.cards.length > 0) {
    ...
    console.log(`   Months covered: ${Math.min(...allLessons.map(l => l.month))} - ${Math.max(...allLessons.map(l => l.month))}`);
