...
async function validateMDXFile(filePath: string): Promise<ValidationResult> {
  ...
  try {
    ...
    if (!validation.success) {
      result.errors.push(
        `Invalid front matter: ${validation.error.issues.map((i) => `${i.path.join(".")}: ${i.message}`).join(", ")}`
      );
      result.success = false;
    }
...
async function validateYAMLFile(filePath: string): Promise<ValidationResult> {
  ...
  try {
    ...
    if (!validation.success) {
      result.errors.push(
        `Invalid flashcard format: ${validation.error.issues.map((i) => `${i.path.join(".")}: ${i.message}`).join(", ")}`
      );
      result.success = false;
    } else {
      // Additional checks
...
async function validateJSONFile(
  ...
  try {
    ...
    if (!validation.success) {
      result.errors.push(
        `Invalid ${isTest ? "test" : "quiz"} format: ${validation.error.issues.map((i) => `${i.path.join(".")}: ${i.message}`).join(", ")}`
      );
      result.success = false;
    } else {
      // Additional checks
      ...
      for (const [index, item] of quiz.items.entries()) {
        if (item.type === "mcq") {
          ...
          if (
            ...
            item.answer < 0 ||
            item.answer >= (item.choices?.length || 0)
          ) {
