...
**Branch:** `feat/cma-2027-build` (13 commits as of the corrected state below, tree clean, not pushed, not merged to `main`).

---

## 0. CORRECTION — the original privacy claim in this document was false

An independent audit (a separate Claude session, invoked by the owner specifically to check this document's claims rather than accept them) found that the "re-verified clean" statement in the original §4.2 below was wrong. The sweep it was based on was scoped only to files touched during the two days of feature work — it never checked the rest of the repository. The same leaked values (the real HBG/Keowee ICP/ICR/note-payable account numbers, documented internally in the workspace's own accounting notes — deliberately not repeated here, since restating them in a file that gets committed would be the same mistake in narrative form) were independently confirmed still present in:

- `REVIEW_QUEUE_CLAUDE.md` (12 occurrences) — a pre-existing internal review-queue doc, untouched by this build
- `docs/CURRICULUM_SPEC.md` (6 occurrences) — a pre-existing spec doc, untouched by this build
- `data/curriculum/cma/m7-w4.json` and `m8-w4.json` (30 and 4 occurrences) — pre-existing CMA Part 2 lesson content, never in scope for the "deepen Part 1" work and never scanned
- this document itself (8 occurrences) — the postmortem repeated the sensitive values verbatim while describing their removal

**All of these predate the two-day build.** `git show main:<file>` and `git show origin/main:<file>` both confirm the numbers were already present in each file before this session started, and a history search on one of the four account numbers alone returns 14 commits across the repo's history — including commits already labeled "Sanitized historical commit" from a prior privacy pass that evidently missed this specific set of numbers. **These values were already live on the public GitHub remote (`origin/main`) before today**, independent of anything built in this session.

One flagged item turned out to be a false positive on closer check: `data/curriculum/cpa/isc/u1-w1.json` and the generated `data/curriculum-cpa.json` both match one of the four numbers only as the trailing digits of "ISO/IEC 27001," a real, correctly-cited security standard unrelated to the leaked account. Confirmed by inspecting all 7 occurrences in context; no fix needed there.

**What changed as a result of the corrected audit:** all five real-leak files above were fixed (whole-file, word-bounded replacement to the same fictional 89010–89013 block used in the original fix), `data/curriculum.json` was rebuilt from the corrected sources, and a repo-wide word-bounded scan now returns zero real hits — the only remaining matches are the confirmed ISO/IEC 27001 references. All gates re-run clean after the fix (type-check, 1,029 tests, content validation).

**What this correction does NOT resolve:** the leak's presence in git history — on this branch, on `main`, and on `origin/main` — is a separate problem from the working-tree content, and working-tree remediation does not remove it from history. That requires a deliberate decision (history rewrite + force-push, or an accepted-risk determination that the values are low-sensitivity account numbers rather than credentials) that is the owner's to make, not something to act on unilaterally. See the end of this document for the specific decision needed.

The original text below is left otherwise unchanged, including its self-assessment, so the record of what was and wasn't checked at each point stays honest rather than quietly cleaned up.
...
**Push/merge decision:** branch is `feat/cma-2027-build`, gated clean, not pushed anywhere and not merged to `main`. That's the owner's call, not something to do automatically.

## 7. The specific decision this correction surfaces

The leaked account numbers are fixed in the current working tree of every branch checked (`main`, `feat/cma-2027-build`), but **they remain in git history on both**, and `main`'s history is already on `origin/main` — a real GitHub remote. Three options, none of which should be executed without the owner choosing one:

1. **Accept the risk.** Account numbers alone (without the real company names, which were never leaked — only fictional names were ever attached to the real numbers) may not meet the bar for a history rewrite. If the owner judges the four numbers as low-sensitivity in isolation, the working-tree fix may be sufficient and this is closed.
2. **Rewrite history on this feature branch only**, before it is ever merged or pushed. Lower blast radius — `feat/cma-2027-build` has never left this machine — but does not touch the pre-existing exposure already on `origin/main`.
3. **Rewrite `main`'s history and force-push** (`git filter-repo` or BFG, then coordinate the force-push). This is the only option that removes the exposure from GitHub. It is also the most disruptive: it rewrites shared history on the actual public remote, invalidates any existing local clones, and needs to be done deliberately, not as a side effect of merging this feature branch.

No history rewrite has been attempted. This section exists so the decision is explicit rather than assumed.
