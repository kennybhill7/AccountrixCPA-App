...
> **Authoring law:** Teach every exam topic **through Ken's real MBG transactions** (Master Brain §7).
...
|---|---|---|---|---|
| w1 | The 4 statements + articulation (BS, IS, equity, cash flows) | MBG GL $12,480,000.00 & Riverton $16,920,000.00 — build each statement from the real TB | `data/knowledge/professor/financial_modeling` | FAR |
| w2 | Asset/liability recognition & measurement | 14xxxxx WIP as a **balance-sheet asset (CIP)** until close — not expense | Master Brain §2; `knowledge/construction` | FAR |
| w3 | Revenue & expense recognition (intro to ASC 606) | Cost-to-cost % completion on a real job; Global Status Report % complete | `data/cpa/modules/module5.json` (Rev Rec) | FAR, BAR |
...
| w4 | Pro-forma financial statements & top-level analysis | Pro-forma close: 5-day monthly / 3-day quarterly SOP | Master Brain §6 | BAR |

## m3 — C. Performance Management (20%)
| Wk | Topic | MBG transaction hook | Salvaged source | Also feeds |
|---|---|---|---|---|
| w1 | Cost & variance analysis (flexible-budget, price/quantity variances) | Job-cost variance: JC-to-GL $390,000.00 on Brookhaven 405 | Master Brain §3.3 | FAR/AUD |
...
| w4 | Balanced scorecard & segment reporting | MBG vs Riverton segment P&L (never combined) | Master Brain §6 rule 4 | BAR |
...
| w3 | **Job-cost reclass** — period/job transfers; gross vs net footing | CD Q1→Q2 reclass: net $31,250.00 vs footing $32,650.00 (the $700 credit) | Master Brain §3.11; `CONTENT/labs` | — |
...
| Wk | Topic | MBG transaction hook | Salvaged source | Also feeds |
|---|---|---|---|---|
| w1 | Governance, risk & compliance (COSO) | COI audit: 49 vendors, 15 expired ($512,000.00), holder-name mismatch | Master Brain §3.7 | AUD |
...
| w3 | **Clearing & suspense / cutoff controls** | Account 111 must net $0; combined $95,000.00 Dr; 102.1 gap $47,200.00 | Master Brain §3.9; `CONTENT/labs/intercompany_matrix` | AUD, FAR |
...
| Wk | Topic | MBG transaction hook | Salvaged source | Also feeds |
|---|---|---|---|---|
| w1 | Information systems (ERP, JCS architecture, data flow) | JCS job-cost GL vs Ledgerline (MF) — crosswalk required | Master Brain §1 | ISC |
| w2 | Data governance & data integrity | "Acct Ending Bal is NOT cumulative"; exact-match not substring | Master Brain §2 scars | ISC, AUD |
...
|---|---|---|---|
| w1 | Liquidity & leverage ratios | MBG working capital, current/quick from real TB | BAR |
...
| Wk | Topic | MBG transaction hook | Also feeds |
|---|---|---|---|
| w1 | **CVP analysis** — breakeven, margin of safety, operating leverage | Job-level contribution margin on a real cost code set | BAR |
...
1. **Cold open — the real MBG problem** (the "hook" above), with the actual numbers.
...
- Every numeric claim labeled **real** (traceable to MBG) or **illustrative**.
...
- **Discipline (pick one):** BAR (Business Analysis & Reporting), ISC (Information Systems & Controls), TCP (Tax Compliance & Planning). *Recommended for Ken: **BAR*** — it reuses the most construction/WIP/ratio content already built.
